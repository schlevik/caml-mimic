...
def write_preds(
    yhat, model_dir, hids, fold, ind2c, yhat_raw=None, save_all_preds=False
):
    """
    INPUTS:
    ...
    if fold != "train" and yhat_raw is not None:
        ...
        for i, (top_idxs, hid) in enumerate(zip(sortd, hids)):
            scores[int(hid)] = {
                ind2c[idx]: float(yhat_raw[i][idx]) for idx in top_idxs[:100]
            }
        with open(scores_file, "w") as f:
            json.dump(scores, f, indent=1)
        if save_all_preds:
            all_scores_file = "%s/pred_all_scores_%s.npy" % (model_dir, fold)
            
            hids_file = "%s/hids_%s.json" % (model_dir, fold)
            i2c_file = "%s/idx2code_%s.json" % (model_dir, fold)
            np.save(all_scores_file, yhat_raw)

            with open(hids_file, "w+") as f:
                json.dump([int(h) for h in hids], f)

            with open(i2c_file, "w+") as f:
                json.dump({int(i): c for i, c in ind2c.items()}, f)
